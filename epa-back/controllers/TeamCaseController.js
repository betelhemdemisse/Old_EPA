...
exports.createTeam = async (req, res) => {
  ...
  try {
    ...
    const complaint = await Complaint.findOne({
      where: { complaint_id },
      include:[{ model: ComplaintAttachement, as: "attachments" },
    ...
    /* ------------------ Create Case ------------------ */
    let caseNo, exists = true;
    while (exists) {
      caseNo = Math.floor(100000 + Math.random() * 900000);
      exists = await Case.findOne({
    ...
    // Support users as array of UUIDs or array of objects { user_id, is_team_leader }
    let normalizedUsers;
    if (users.length > 0 && typeof users[0] === "object" && users[0] !== null) {
      normalizedUsers = users.map(u => ({
        user_id: u.user_id,
        is_team_leader: !!u.is_team_leader,
      }));
    } else {
      normalizedUsers = users.map(u => ({ user_id: u, is_team_leader: false }));
      if (leader_user_id) {
        normalizedUsers = normalizedUsers.map(u => ({
          ...u,
          is_team_leader: u.user_id === leader_user_id,
        }));
      }
    }

    const leaderCount = normalizedUsers.filter(u => u.is_team_leader).length;
    if (leaderCount !== 1) {
      await transaction.rollback();
      return res.status(400).json({ message: "Exactly one team leader must be selected." });
    }

    const userIds = normalizedUsers.map(u => u.user_id);
    ...
    const teamMembers = normalizedUsers.map(({ user_id, is_team_leader }) => ({
      user_id,
      case_id,
...
exports.addUserToTeam = async (req, res) => {
  const transaction = await sequelize.transaction();
  try {
    const { case_id, users } = req.body; // users can be [{ user_id, is_team_leader? }] or [uuid]
    const currentUser = req.user.id;
    ...
    if (!userRole) {
      await transaction.rollback();
      return res.status(403).json({ message: "You are not assigned to any role." });
    }
    ...
    const existingTeam = await TeamCase.findAll({ where: { case_id }, transaction });
    ...
    let normalizedUsers;
    if (users.length > 0 && typeof users[0] === "object" && users[0] !== null) {
      normalizedUsers = users.map(u => ({ user_id: u.user_id, is_team_leader: !!u.is_team_leader }));
    } else {
      normalizedUsers = users.map(u => ({ user_id: u, is_team_leader: false }));
    }

    const admins = await AdministratorAccounts.findAll({ where: { user_id: normalizedUsers.map(u => u.user_id) }, transaction });
    if (admins.length !== normalizedUsers.length) {
      await transaction.rollback();
      return res.status(400).json({
    ...
    const filteredUsers = normalizedUsers.filter(u => !existingUserIds.includes(u.user_id));
    ...
    // If any incoming user is marked as leader, demote current leader first (only one leader per case)
    const wantsLeader = filteredUsers.find(u => u.is_team_leader);
    if (wantsLeader) {
      await TeamCase.update({ is_team_leader: false }, { where: { case_id, is_team_leader: true }, transaction });
    }

    const newMembers = filteredUsers.map(({ user_id, is_team_leader }) => ({
      user_id,
      case_id,
    ...
    await TeamCase.bulkCreate(newMembers, { transaction });
...
exports.removeUserFromTeam = async (req, res) => {
  ...
  try {
    ...
    const userRole = await UserHasRole.findOne({ where: { user_id: currentUser } });
    if (!userRole) { await transaction.rollback(); return res.status(403).json({ message: "You are not assigned to any role." }); }
    ...
    // Prevent removing the only leader without reassigning first
    const leaders = await TeamCase.findAll({ where: { case_id, is_team_leader: true }, transaction });
    const removingLeader = teamMembers.find(m => m.is_team_leader);
    if (removingLeader && leaders.length === 1) {
      await transaction.rollback();
      return res.status(400).json({ message: "Cannot remove the only team leader. Assign another leader first." });
    }
...
exports.getTeamMembers = async (req, res) => {
  try {
    ...
    const teamMembers = await TeamCase.findAll({
      ...
      order: [["is_team_leader", "DESC"], ["created_at", "ASC"]]
    });
