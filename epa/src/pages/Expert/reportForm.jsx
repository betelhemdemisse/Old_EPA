...
export default function DynamicFormPage() {
  ...
  const { report_type_id, case_id } = location.state || {};
  ...
  const loadForm = async () => {
    try {
      ...
      if (res?.status && res.data?.reportingForm) {
        setFormFields(res?.data?.reportingForm);
        const initialValues = {};
        res?.data?.reportingForm.forEach((field) => {
          if (field.input_type === "checkbox") {
            initialValues[field.report_form_id] = [];
          } else {
            initialValues[field.report_form_id] = "";
          }
        });
      ...
      console.error("Error loading dynamic form", err);
    }
  };

  useEffect(() => {
    if (report_type_id) loadForm();
  }, [report_type_id]);

  // Group fields by form type with modern structure similar to image
  ...
  const handleSubcategoryChange = (e) => {
    ...
    if (selectedValue) {
      ...
      const foundSubcategory = subcategories.find(
        (sub) => sub.issue_type === selectedValue
      );
      if (foundSubcategory) {
        setSubcategoryId(foundSubcategory.penality_sub_category_id);
      }
    } else {
      setSubcategoryId("");
    }
  ...
  const getSubcategories = () => {
    ...
    const category = penalityList.data?.find(
      (item) => item.penalty_id === selectedCategory
    );
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (selectedCategory) {
        formData.append("penalty_category_id", selectedCategory);
      }

      if (subcategoryId) {
        formData.append("penality_sub_category_id", subcategoryId);
      }

      closingAttachments.forEach((file) => {
  ...
  // Render field based on input type - Modern version
  const renderField = (field) => {
    const value = formValues[field.report_form_id] || "";
    const isCheckbox = field.input_type === "checkbox";

    // Determine label with asterisk for required fields
    const labelText = field.report_form !== "none" ? field.report_form : "";
    const isRequired = field.report_form?.includes('*');
        ...
        <div className="relative">
          ...
          {(field.input_type === "text" || field.input_type === "number") && (
            <input
              ...
              value={value}
              onChange={(e) => handleChange(field.report_form_id, e.target.value)}
                ...
                <label key={option} className="flex items-center cursor-pointer group">
                  <div className="relative">
                    ...
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center
                      ${value === option ? 'border-green-500' : 'border-gray-300 group-hover:border-green-400'}`}>
                      {value === option && (
              ...
              {field.options?.map((option) => {
                ...
                return (
                  <button
                    key={option}
                    type="button"
                    onClick={() => handleChange(field.report_form_id, option, true)}
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                    ...
                    return (
                      <div key={sectionKey} className="flex items-center">
                        ...
                        <span className={`ml-3 font-medium ${isActive ? 'text-gray-900' : 'text-gray-500'}`}>
                          {groupedFields[sectionKey].name}
                        </span>
                        {index < sectionKeys.length - 1 && (
                          <div className={`h-0.5 w-8 mx-4 ${isCompleted ? 'bg-green-600' : 'bg-gray-200'}`}></div>
                        )}
                      </div>
                    );
                  })}
              ...
              <div className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {groupedFields[sectionKeys[currentStep]]?.fields.map(renderField)}
                </div>
                ...
                <div className="flex justify-between mt-12 pt-8 border-t border-gray-100">
                  <button
                    ...
                    className={`px-8 py-3.5 rounded-xl font-medium transition-all duration-200
                      ${currentStep > 0 
                  ...
                  {currentStep < totalSteps - 1 ? (
                    <button
                      onClick={handleNext}
                      className="px-8 py-3.5 bg-green-600 text-white font-medium rounded-xl
            ...
            <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 sticky top-6">
              ...
              <div className="space-y-4">
                {sectionKeys.map((sectionKey, index) => (
                  <div key={sectionKey} className="border border-gray-200 rounded-xl overflow-hidden">
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-2 h-2 rounded-full ${index === currentStep ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        <span className="font-medium text-gray-800">{groupedFields[sectionKey].name}</span>
                      </div>
                      <svg 
                        className={`w-5 h-5 text-gray-400 transition-transform duration-300 ${expandedSections[sectionKey] ? 'rotate-180' : ''}`}
                        fill="none" 
                        stroke="currentColor" 
                        viewBox="0 0 24 24"
                      >
                    ...
                    <div className={`overflow-hidden transition-all duration-300 ${expandedSections[sectionKey] ? 'max-h-96' : 'max-h-0'}`}>
                      <div className="p-4 space-y-3">
                        {groupedFields[sectionKey].fields.map((field) => {
                          const value = formValues[field.report_form_id];
                          return (
                            <div key={field.report_form_id} className="pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                              <p className="text-sm font-medium text-gray-700 mb-1">{field.report_form}</p>
                              {field.input_type === 'checkbox' ? (
                                <div className="flex flex-wrap gap-1.5">
                                  {(value || []).map((val, idx) => (
                                    <span key={idx} className="inline-block bg-green-50 text-green-700 px-2.5 py-1 rounded-lg text-xs">
                                      {val}
                                    </span>
                                  ))}
                                  {(value || []).length === 0 && (
              ...
              {(selectedCategory || selectedSubcategory) && (
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <h4 className="font-medium text-gray-800 mb-3">Penalty Details</h4>
                  <div className="space-y-3">
                    {selectedCategory && (
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Category:</span>
                        <span className="font-medium text-green-600">{getSelectedCategoryName()}</span>
                      </div>
                    )}
                    {selectedSubcategory && (
                      <div className="flex items-center justify-between">
                        <span className="text-sm text-gray-600">Subcategory:</span>
                        <span className="font-medium text-blue-600">{selectedSubcategory}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {closingAttachments.length > 0 && (
                <div className="mt-6 pt-6 border-t border-gray-200">
                  ...
                  <div className="space-y-2">
                    {closingAttachments.slice(0, 3).map((file, index) => (
                    ...
                    {closingAttachments.length > 3 && (
                      <p className="text-xs text-gray-500">+ {closingAttachments.length - 3} more files</p>
                    )}
                  </div>
                </div>
              )}
